...
class NMR2DMPNN(nn.Module):
    """
    Thin wrapper so the 2D baseline can be called as: pred = model(data) -> pred[data.mask]
    """
    def __init__(self,
                 node_in_dim: int = 74,
                 edge_in_dim: int = 9,
...
class NMR3DNet(nn.Module):
    ...
    def forward(self, data):
        """
        data: PyG Data with x, edge_index, edge_attr, pos(optional), mask, batch
        """
        # ========== 2D BACKBONE ==========
...
#     def forward(self, data):
#         """
#         data: PyG Data with x, edge_index, edge_attr, pos, mask, batch
#         """
#         # Project inputs
#         x_init = self.node_proj(data.x)
#         edge_features = self.edge_proj(data.edge_attr)
#
#         # === 2D Branch (no 3D features) ===
#         x_2d = x_init
#         for layer in self.schnet_2d_layers:
#             x_2d = layer(x_2d, data.edge_index, edge_features)
#
...
#                 x_schnet_3d = layer(x_schnet_3d, data.edge_index, edge_features_3d)
#
#             # EGNN
#             x_egnn = x_init
#             for layer in self.egnn_layers:
#                 x_egnn = layer(x_egnn, data.pos, data.edge_index, edge_features_3d)
#
#             # Fuse 3D components
...
#
#         # === Adaptive Mixing ===
#         if self.use_adaptive and hasattr(data, 'pos'):
#             # Learn per-atom mixing weights based on atom features
#             mixing_weights = self.selector(data.x)  # [N, 1]
#             x_out = mixing_weights * x_3d + (1 - mixing_weights) * x_2d
#         else:
#             # If adaptive disabled, use 3D only (or 2D if no pos)
#             x_out = x_3d if hasattr(data, 'pos') else x_2d
#
#         # Graph-level readout
#         graph_features = self.set2set(x_out, data.batch)
#         graph_features_per_atom = graph_features[data.batch]
#
#         # Prediction
#         combined = torch.cat([x_out, graph_features_per_atom], dim=-1)
#         predictions = self.predictor(combined).squeeze(-1)
#         predictions = predictions[data.mask]
#
#         return predictions
#
#     def get_mixing_weights(self, data):
#         """Get the learned 2D/3D mixing weights for analysis"""
#         if not self.use_adaptive:
#             return None
#         with torch.no_grad():
#             weights = self.selector(data.x)
#             return weights[data.mask].cpu().numpy()

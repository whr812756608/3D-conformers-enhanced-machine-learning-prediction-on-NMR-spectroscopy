...
def evaluate_ensemble_with_atoms(model_2d, model_3d, test_set, conformer_folder, device):
    """Evaluate ensemble using ALL conformers per molecule; keep only improved atoms for listing"""
    ...
    for idx, data in enumerate(test_set):
        ...
        # Atom-level rows (only improvements kept)
...
def analyze_top_predictions(df_atoms, top_n=30):
    """Print top-N improved atoms only"""
    print(f"\n{'=' * 80}")
...
def main():
    ...
    path_2d = os.path.join(MODEL_DIR, "H_2D_baseline.pt")
    path_3d = os.path.join(MODEL_DIR, "H_3D_fixed.pt")
    split_path = os.path.join(MODEL_DIR, "H_split_indices.npz")

    # Build H dataset
    print("\n" + "=" * 80)
    print("LOADING PROTON (1H) NMR DATASET")
    print("=" * 80)
    ...
    if not os.path.exists(split_path):
        raise FileNotFoundError(f"Split indices not found at {split_path} (run train_H.py first).")
    ...
    model_2d.load_state_dict(torch.load(path_2d, map_location=device))
    model_2d.eval()
    ...
    model_3d.load_state_dict(torch.load(path_3d, map_location=device))
    model_3d.eval()

    # 2D & 3D-fixed test MAEs
    ...
    # Multi-conformer ensemble on the same test set
    print(f"\n{'=' * 80}\nMULTI-CONFORMER ENSEMBLE EVALUATION (H)\n{'=' * 80}")
    df_atoms, df_mols = evaluate_ensemble_with_atoms(
        model_2d, model_3d, test_set, cfg.conformer_folder, device
    )

    mae_3d_ensemble = df_mols["mae_3d_ensemble"].mean() if len(df_mols) else float('nan')
    print(f"\n{'=' * 80}\nSUMMARY\n{'=' * 80}")
    ...
    if np.isfinite(mae_3d_ensemble):
    ...
    if len(df_atoms):
        ...
        print("\n(No per-atom improvements recorded — check conformer files/matching.)")

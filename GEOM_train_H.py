...
def train_model(config_name, model, dataset, device, train_set, val_set, test_set,
    ...
    test_pred, test_true = evaluate(model, DataLoader(test_set, batch_size=32, shuffle=False), device)
...
def main():
    ...
    # Optional 3D pretrain (set to True and update path if used)
    PRETRAINED_PATH = "./GEOM_pretrain/pretrained_models/H3D_pretrained.pt"
    ...
    # Fixed split; save indices for eval reproducibility
    n = len(dataset)
    torch.manual_seed(42)
    train_set, val_set, test_set = torch.utils.data.random_split(
        dataset, [int(0.8 * n), int(0.1 * n), n - int(0.8 * n) - int(0.1 * n)]
    )
    split_path = "./GEOM_model/H_split_indices.npz"
    np.savez(
        split_path,
    ...
    model_2d, val_2d, test_2d = train_model(
        "H_2D_baseline", model_2d, dataset, device, train_set, val_set, test_set,
        is_adaptive=False
    )

    # ===== Train 3D fixed =====
    print(f"\n{'=' * 80}\nTRAINING 3D FIXED MODEL (H)\n{'=' * 80}")
    model_3d = NMR3DNet(
    ...
    model_3d, val_3d, test_3d = train_model(
        "H_3D_fixed", model_3d, dataset, device, train_set, val_set, test_set,
        is_adaptive=False
    )

    print("\nDone training. Saved:")
    print("  ./GEOM_model/H_2D_baseline.pt")
    print("  ./GEOM_model/H_3D_fixed.pt")
